...
//start NCSU
const char kJsApiUserManagerAddUserIncognito[] = "addUser";
//end NCSU
...
//start NCSU
void UserManagerScreenHandler::HandleAddUserIncognito(const base::ListValue* args) {
    if (!IsAddPersonEnabled()) {
        // The 'Add User' UI should not be showing.
        NOTREACHED();
        return;
    }
    profiles::CreateAndSwitchToNewProfileIncognito(
        desktop_type_,
        base::Bind(&UserManagerScreenHandler::OnSwitchToProfileComplete,
        weak_ptr_factory_.GetWeakPtr()),
        ProfileMetrics::ADD_NEW_USER_MANAGER);
...
//start NCSU
    web_ui()->RegisterMessageCallback(kJsApiUserManagerAddUserIncognito,
      base::Bind(&UserManagerScreenHandler::HandleAddUserIncognito,
                base::Unretained(this)));
//end NCSU
