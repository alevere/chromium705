...
//start NCSU allow temp dir to be defined
...
//start NCSU handle incognito tabs by creating new profile
//again or do we manipulate the cookie store alone in existing profile
  else {
      //base::string16 mytitle(base::ASCIIToUTF16("alex"));
      //web_ui->OverrideTitle(mytitle);
      //scoped_ptr<NewTabHTMLSource> html_source(new NewTabHTMLSource(profile->GetOriginalProfile()));
      //html_source->AddResource("../user_manager/user_manager.js", "application/javascript",IDR_USER_MANAGER_JS);
      //html_source->AddResource("../user_manager/user_manager.html", "application/javascript",IDR_USER_MANAGER_HTML);
      //web_ui->AddMessageHandler(new options::BrowserOptionsHandler());
      //web_ui->
      //web_ui->AddMessageHandler(new UserManagerUI());
      web_ui->AddMessageHandler(new UserManagerScreenHandler());
      //above line works with chrome.send('addUser') in incognito_tab.html
      //but, it opens a new non-incognito window. Appears to eventually call ProfileManager::CreateMultiProfileAsync()
      //in profile_manager.h from profile_window.h
...
//start NCSU create a new profile
//or should we just create a new cookie store?
//it doesnt seem a cookie store is possible.

Profile* NewTabUI::CreateNewProfile() const {
    //base::FilePath path_profile;
    //path_profile = ProfileManager::GetGuestProfilePath();
    //base::string16 prof_name(base::ASCIIToUTF16("alex"));
    //base::string16 prof_icon(base::ASCIIToUTF16("alex"));
    return Profile::FromWebUI(web_ui());
    //MockProfileDelegate *mydelegate = new MockProfileDelegate;
    //return Profile::CreateProfile(path_profile, mydelegate, Profile::CREATE_MODE_SYNCHRONOUS);
    //ProfileManager *ted = new ProfileManager(path_profile);
    //ProfileManagerWithoutInit::CreateCallback callback;
    //path_profile = ted->CreateMultiProfileAsync(prof_name, prof_icon, callback, prof_supv);
    //path_profile1 = ProfileManager::CreateMultiProfileAsync(prof_name,prof_icon, callback, prof_supv);
    //return ted->GetProfile(path_profile);
    //web_ui()->RegisterMessageCallback("createProfile",base::Bind(&AsyncWebUIMessageHandler::HandleStartAsyncTest,base::Unretained(this)));
    //ProfileManager myprofile = ProfileManager::ProfileManager(path_profile1);
    //base::FilePath path_profile2 = myprofile.GenerateNextProfileDirectoryPath();
    //return 0;
    //Profile::Delegate delegate;
    //base::ScopedTempDir temp_dir;
    //result_a=temp_dir.CreateUniqueTempDir();
    //Profile* profile = NULL;
    //return CreateOffTheRecordProfile();
    //return profile;
    //Profile myprofile;
    //Profile* CreateProfile(const base::FilePath& path,Delegate* delegate,CreateMode create_mode);
    //return Profile::CreateProfile(NULL, Profile::CREATE_MODE_SYNCHRONOUS);
    //return ProfileManager::CreateAndInitializeProfile(temp_dir.path());
    //ProfileManager newprof;
    //return newprof.GetProfile(temp_dir_.path().AppendASCII("test"),);
}
//end NCSU
///////////////////////////////////////////////////////////////////////////////
// NewTabHTMLSource
